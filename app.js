...
let sqlCreateTabe = "CREATE TABLE IF NOT EXISTS `files` (`id` INT(11) NOT NULL AUTO_INCREMENT,`name` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci,`extension` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci,`mime_type` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci,`size` DOUBLE,`date_downloaded` TIMESTAMP, PRIMARY KEY (`id`)) ENGINE=InnoDB;"
connection.query(sqlCreateTabe, function(err, rows) {
    if (err) {
        return console.error(err)
    }
})
sqlCreateTabe = "CREATE TABLE IF NOT EXISTS `users` (`userId` INT(11) NOT NULL AUTO_INCREMENT,`id` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci,`password` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci,`refreshToken` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci, PRIMARY KEY (`userId`)) ENGINE=InnoDB;"
connection.query(sqlCreateTabe, function(err, rows) {
    if (err) {
        return console.error(err)
    }
...
expressApp.post('/signup', function(req, res) {
    ...
    sql = `INSERT INTO users (id, password, refreshToken) VALUES (?, ?, ?)`
    connection.query(sql, data, function(err, rows) {
        if (err) {
            return console.error(err)
        }
